...
Kotlin SDK for the Proxmox Datacenter Manager API. Generated
...
repositories {
    maven {
        url = uri("https://maven.pkg.github.com/client-api/pdm-kotlin")
        credentials {
...
val status = pdm.qemu().qemuVmStatus(node = "pdm1", vmid = 100L)
val nodes  = pdm.nodes().nodesGetNodes()
```
