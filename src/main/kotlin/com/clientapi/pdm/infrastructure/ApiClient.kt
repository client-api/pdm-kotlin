package com.clientapi.pdm.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val BASE_URL_KEY: String = "com.clientapi.pdm.baseUrl"
        @Deprecated(
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
            if (apiKey["PDMAuthCookie"] != null) {
                if (apiKeyPrefix["PDMAuthCookie"] != null) {
                } else {
                }
            }
        if (requestConfig.headers["CSRFPreventionToken"].isNullOrEmpty()) {
            if (apiKey["CSRFPreventionToken"] != null) {
                if (apiKeyPrefix["CSRFPreventionToken"] != null) {
                    requestConfig.headers["CSRFPreventionToken"] = apiKeyPrefix["CSRFPreventionToken"]!! + " " + apiKey["CSRFPreventionToken"]!!
                } else {
                    requestConfig.headers["CSRFPreventionToken"] = apiKey["CSRFPreventionToken"]!!
                }
            }
        }
